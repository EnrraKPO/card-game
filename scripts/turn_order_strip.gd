...
var _marked: CardInstance = null     # the cursor is on this unit's CARD
var _selected: CardInstance = null   # this unit is the pick — the inverted skin (see _dress)
...
func _make_entry(inst: CardInstance, number: int) -> Control:
	...
	entry.mouse_exited.connect(func() -> void:
		if _hovered == inst:
			_set_hovered(null))
	entry.gui_input.connect(func(event: InputEvent) -> void: _on_entry_input(event, inst))
...
func _sync_attention() -> void:
	...
	_set_selected(Selection.current() as CardInstance)
...
func _set_selected(inst: CardInstance) -> void:
	if inst == _selected:
		return
...
func mark(inst: CardInstance) -> void:
	_forced = inst != null
	_set_marked(inst)
...
# own, it is another place the same units can be reached from. So the press goes to the board and
# comes out wherever a press on the card itself would have (see CombatBoard.press_unit): the
...
var _pressed_on: CardInstance = null

func _on_entry_input(event: InputEvent, inst: CardInstance) -> void:
	...
	if mb.pressed:
		_pressed_on = inst
		return
	var here := _pressed_on == inst
	_pressed_on = null
	if here and board != null:
		board.press_unit(inst)


func _entry_for(inst: CardInstance) -> Control:
	if inst == null or _rows == null:
		return null
	for child in _rows.get_children():
		var entry := child as Control
		if entry.get_meta("inst") == inst:
			return entry
	return null
