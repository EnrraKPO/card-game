...
func _do_cpu_placement() -> void:
	...
	# placeholder AI is deleted. All four action kinds are planned: placements, moves, spell
	# casts and ability activations (the latter two simulate on copies of the live world
	# through the REAL rules — see CandidateApply; while targeting is demolished,
	# can_simulate_cast refuses everything, so the CPU plans placements and moves only).
...
func _run_combat() -> void:
	...
	for attacker: CardInstance in _world.turn_order():
		...
		# moment IS the whole turn: the one dispatch unfolds everything that clings to it —
		# poison procs, the unit's own MAIN ACTION (fired by its appointment, gated untapped —
		# signed MAIN_ACTION_DESIGN.html §4), status decay. There is
...
# after the result cue's readout returns, and cues that need this gate can run INSIDE it — waiting
# on the cleanup would deadlock. The tween runs on the scene clock, independent of the await chain.
