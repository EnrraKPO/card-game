...
# THE WIDGET NEVER READS THE GAME WORLD (docs/planning/RULINGS.html R4). Every fact it renders
# is injected: the host stamps `own_side` at construction, occupancy arrives through
# set_card/clear_card, cues through set_cue and its siblings, and the ground through
# set_ground(SlotGroundView). The widget stores only presentation facts it is the authority
# on; whoever composes its inputs is the one piece that speaks to the engine.

# Which half of the board this slot belongs to — the player's own half shows placement hints
# (the idle OPEN marker) and the enemy's never does. Injected once by the host at construction.
var own_side: bool = false
...
# Cursor-hover during a STATIC moving/placing selection (see the host's hover tracking):
...
# placeholders swappable for PNGs (see BoardGlyphs). The host drives the state; SlotUI just
...
# own move cues. The red glow that pairs with it rides the target CARD via Vfx (host's call).
var _attack_icon: TextureRect
# A preview of the unit that would land in THIS slot if the drag were released now (the host's
# drag phantom). Rendered as a holographic PROJECTION — cooler, brighter and less saturated
# than the card really is — so it reads clearly apart from the warm, dim, translucent ghost the
# cursor drags (DragGhost.GHOST_TINT). The look is CardUI.set_phantom, the one shared "this
# card isn't real" treatment (a colour transform of the card's own pixels — see
# phantom.gdshader). This used to hand-roll a wash rect here, on the belief that a per-node
# shader blanks the card's clipped, COVER-fit art node; measured, it doesn't. Mounted/unmounted
# by the host.
var _phantom: CardUI = null

# Translucency is a SEPARATE axis from the phantom colour and stays local: it says how PRESENT
# the projection is, and the answer here is "more present than the see-through drag ghost".
...
# or the corner crosshair that may share the same card. Empty valid slots have no card to light,
# so they fall back to the reticle glyph. Tracked so we detach from the exact card that was lit,
# even if occupancy shifts before the gesture clears.
const TARGET_VALID_GLOW := "target_valid_glow"
var _glow_card: CardUI = null


# ── Ground state (injected — see SlotGroundView) ────────────────────────────────
# The ground layer renders whatever SlotGroundView the host last injected (set_ground); the
# widget stores that view and nothing else about the ground. Null = no ground.
...
#
# The board gutter — the separation between slots. It lived on the old board-data layer; the
# widget now owns the number, and ANY host laying out a grid of slots must use this same gap,
# or the ground frames below stop tiling.
const SLOT_GAP := 14
# The frame claims exactly HALF of each surrounding gap, so two adjacent slots carrying the
# same ground status meet with no seam and a spreading status reads as ONE FIELD rather than a
# row of outlined boxes. That continuity is the point of the number: a narrower rail was tried
# and left a channel of bare board between neighbours. Half the gap also fits the board's edge
# slots with no special case — a zone insets its grid by half a gap plus one pixel.
const GROUND_FRAME_REACH := float(SLOT_GAP) * 0.5
# SHARP. A rounded frame was concentric with the slot's own 5px rounding and looked right in
# isolation, but it is the corners that decide whether a spread ground is one field or four
# tiles: two rounded frames meeting in a gutter leave a notch at every junction, and the region
# reads as beads on a string. Square corners tile exactly, edge to edge and corner to corner.
...
# constant (it does not scale with the slot); short of it by GROUND_ROW_CLEARANCE so the icon's
# top stops a visible hair from the card above — "almost scraping, never touching".
const GROUND_ROW_CLEARANCE := 2.0
const GROUND_ROW_RISE := float(SLOT_GAP) - GROUND_ROW_CLEARANCE
# How much of the slot's width ONE tab takes when it is alone. Half the tile was tried and read
# as too broad; this is two thirds of that — wide enough to be a plate rather than a chip,
# without the lone tab looking like a second health bar. Every other count falls out of the
# equal share below.
...
var _ground: SlotGroundView = null
var _ground_frame: Panel = null
var _ground_floor: Panel = null
# The light this ground casts on whoever stands in it — WHITE (a multiply no-op) when there is
# no ground to cast any. Held because occupancy and ground change independently, and whichever
# moves has to be able to re-assert the other's fact without a fresh injection.
...
# The occupant scale the tab row was last laid out FOR — the widget's record of what it drew,
# not a copy of anyone else's state (the card stays the authority; this is change detection).
...
func _build_ground_layer() -> void:
	...
	# NOTE: the ground's LIGHT on its occupant is not a node here at all — it is a tint handed to
	# the card, which multiplies it over its own picture (CardUI.set_ground_tint). It lived here
	# first, as a multiply Panel over the slot's rect, and that shape cannot work: the stat badges
	# overflow the card's rect, so a rect-shaped pass tints them (wrong — they are UI, not
	# scenery) and still cannot cover them fully. The ground says WHAT light falls; the card
...
# Injects the ground's display facts and re-renders the layer. Null clears it. The widget keeps
# the view only to re-derive layout on resize — it never asks anyone what the ground holds.
func set_ground(view: SlotGroundView) -> void:
	_ground = view
	_render_ground()


# Re-derives the ground presentation from the injected view: the gutter frame and the floor wash
# in the leading status's own colour, its ambient VFX, and one StatusPip per ground status — the
# same pip, tooltip and cue vocabulary units use, so "the ground has a status" reads exactly
# like "a unit has one".
func _render_ground() -> void:
	for child in _ground_pips.get_children():
		_ground_pips.remove_child(child)
		child.queue_free()
	if _ground == null or _ground.pips.is_empty():
		_ground_frame.visible = false
		_ground_floor.visible = false
	...
	fs.bg_color = Color(_ground.color, 0.0)
	fs.border_color = GroundPalette.frame(_ground.color)
	...
	gs.bg_color = GroundPalette.floor_wash(_ground.color)
	...
	_ground_tint = GroundPalette.on_occupant(_ground.color)
	_sync_ground_vfx(_ground.status_id)   # the tint is pushed by _layout_ground below
	for pv: StatusPipView in _ground.pips:
		# A duplicates-display status shows ONE TAB PER STACK — the row literally counts them —
		# each tab icon-only (`solo`). The default shows one tab wearing the count. Reshaped only
		# once IN the tree: the tab lays its count label out from text metrics, which read
		# near-zero until the node can resolve a theme.
		for _i in maxi(pv.stacks if pv.duplicates else 1, 1):
			var pip := STATUS_PIP_SCENE.instantiate() as StatusPip
			pip.setup(pv)
			_ground_pips.add_child(pip)
			pip.as_ground_tab(pv.duplicates)
	_layout_ground()
...
#
# THE ROW WEARS ITS OCCUPANT'S TRANSFORM. A selected unit grows about its own centre — which,
# since the card is anchored to fill the slot, is the SLOT's centre — and a tab row that stayed
# put would be swallowed by the card climbing over it. So the row takes the same scale about the
# same pivot, and its rise falls out of that for free rather than being a second rule: sitting
# above the pivot, scaling lifts it. What that preserves is the PROPORTION visible above the
# card's top edge (11px of 19 at rest, 11.7 at the authored 1.07) — the tabs read exactly as
# well grown as resting, at any factor. Stated as "match the occupant", not "on selection", so
# any future grow inherits it.
func _layout_ground() -> void:
	if _ground_pips == null:
		return
	var tabs := _ground_pips.get_children()
	if not tabs.is_empty():
		# Width is a share of the SLOT, never an authored pixel count: a tab is as wide as the
		# space it has, so the row reads the same on any slot size. One tab takes
		# GROUND_TAB_WIDTH_FRAC of the usable width; from two upward the equal share is the
		# smaller number and takes over, so the progression is continuous and the row NEVER
		# bleeds into the neighbouring column.
		...
		# The fraction is of the SLOT's full width, not of `inner` — "half the tile" is the thing
		# the eye actually judges, and the end insets are breathing room rather than part of the
		# measure.
	...
	# small inset in from it. Right-aligned rather than centred so the row has a fixed end to
	# build from — tabs are appended, so the newest lands at the pinned edge and the pile grows
	# leftward, and nothing already on screen shifts sideways just because the count changed. (A
	# centred row re-centres on every stack, so the whole pile slides half a tab each time the
	# count ticks.)
	...
	# ONE owner for the occupant's tint, and it is here rather than in _render_ground because
	# occupancy changes WITHOUT the ground changing at all — a unit walking onto claimed ground
	# never re-renders it. Every path that alters either fact ends in this function.
	if _card_ui != null and is_instance_valid(_card_ui) 			and _card_ui.has_method(&"set_ground_tint"):
...
# moves as ONE. They used to stay put on the reasoning that terrain does not grow because a unit
# was picked, which was right while the frame was a broad band; a thin rail left behind by its
# own tabs just looks broken. The floor comes along for internal consistency: a frame that steps
# outward while the floor holds still opens a ring of bare slot between them.
# Both rects are CENTRED on the slot's centre already (the frame is the slot grown evenly on
# every side), so scaling about each one's own middle is the same transform the card gets — no
# position arithmetic needed, unlike the tab row, which sits off-centre.
func _scale_ground_surfaces(s: float) -> void:
	...
	# (The occupant's tint needs no transform of its own — it rides the card's art node, so it
	# grows and moves with the card by construction.)
...
# The follow has to be CONTINUOUS, not endpoint-synced: the grow tween is TRANS_BACK and
# overshoots past its target before settling, so a row that only learned the final scale would
# visibly lag and then snap. Polling the CARD's transform is presentation reading presentation —
# the card stays the authority on its own scale and this stores nothing but what it last drew —
# and it costs a float compare on the slots that actually have tabs to move.
...
# one channel on the board with no other claimant, and it is the only one that still reads when
# a card covers the slot outright — because emitted particles are DELIBERATELY exempt from the
# occlusion rule above (user's call, and the right one: a ground effect that stops dead at a
# card's edge reads as broken). The emit behavior draws on the overlay layer, above the board,
# so particles fly wherever they like for free.
#
# CONVENTION, never a per-status special case. A ground status animates through up to three
# library entries, each optional and independently gated:
#   ground_<status_id>         — the ambient PARTICLES thrown off the ground
#   ground_<status_id>_haze    — the ambient FIELD (the air over it)
...
# doesn't are different statements, and a status may make both. Authoring these is the whole
# cost of giving the next ground status its motion — one might write `_haze` alone as a blur
# and skip the particles; a status with none of the three simply sits still.
...
# restart them, or a claimed slot would flicker its way through the whole fight.
...
# The loud half of the ground's motion, fired in concert with a tab's glint: the same particles
# the ambience emits, in a burst. One vocabulary, two volumes — the shape hover and the turn
# spotlight already share.
...
# a stack that faded mid-pass removed its tab while later rolls in the snapshot were still due.
...
# ── The pointer ring ────────────────────────────────────────────────────────────
# THE SLOT ANSWERS ONLY WHILE IT IS EMPTY. The rule for the board is "outline whatever the
# pointer is actually addressing", and an occupied slot is not that — the thing you mean is the
# unit standing in it, and its card wears the ring itself (see CardUI._apply_hover_outline). An
# EMPTY slot is the thing you mean, a place to put something, and it was the one board surface
# that answered a cursor with nothing at all.
#
# `mouse_entered` is trustworthy here in a way it is not for a lifting hand card: a slot is
# furniture, it never moves out from under the pointer, so there is no oscillation to hit-test
# around.
...
# Derived from both facts every time, never toggled from one: a unit placed into (or killed out
# of) the slot the cursor happens to be resting on hands the ring over without either side
# noticing the other. Called from set_card/clear_card for exactly that.
...
func _apply_style() -> void:
	...
	# app's plastic-toy palette), but an empty battlefield slot is the game board, not UI chrome;
	# it needs to stay a dark, receding "empty" surface so cards read clearly against it either
	# way. Still themeable via ScreenUI.SLOT_* (backed by UIPalette's own "Combat board" group).
...
# MOVE cue ("reposition here") composition. The spot is a foreshortened ellipse — a spotlight
# pool seen in perspective — biased toward the slot's lower edge, with the chunky arrow bobbing
# just above it. All relative to the slot so it reads at any board scale.
...
# Sizes/positions the glyphs relative to the slot's current size (hosts resize slots to fill
...
# _apply_icon_geometry, since a stale _spot_top (e.g. from the NONE state) would fling it
# off-slot.
...
# The attack crosshair COMPOSES with any centre cue instead of colliding with it — a slot can
# show several states at once. Alone, the crosshair sits centred (the classic "this is the
# target" read). When a centre glyph is ALSO up (e.g. the green reticle on an enemy that is also
...
func set_cue(cue: int, animated: bool = false) -> void:
	...
	# host re-raises it after the MOVE cue when the action wants it (see set_move_button).
	...
	match cue:
		...
		Cue.MOVE:
			...
			_layout_arrow()   # _apply_icon_geometry (above) just set _spot_top — place the arrow
			_arrow.position.y = _arrow_top_y
			if animated:
				_restart_arrow_bob()
		Cue.TARGET_OK:
			# An occupied valid target lights the CARD from within (golden inner glow) rather
			# than stamping the reticle over it; an empty valid slot has nothing to light, so it
			# keeps the reticle glyph.
			if _card_ui != null:
				_light_valid_glow()
				_icon.visible = false
			else:
...
# changes.
func reset_cue() -> void:
	if _open_hints and _card_ui == null and own_side:
		set_cue(Cue.OPEN)
	else:
		set_cue(Cue.NONE)


# Raises/retires the move button on a MOVE-cue slot — called by the host for static,
# non-click-commit DESTINATIONS (a selected fielded unit's landing spots). The button REPLACES
# the bare MOVE glyphs (it renders its own spot+arrow inside its frame); hiding it cancels any
# in-flight hold and releases its hover (MoveButton's visibility notification).
...
# a live drag shows; the resting static cue is spotlight-only). Driven by the host's hover
...
# Lights the current occupant from within as a valid target (golden inner glow). Tracks the
# exact card lit so _clear_valid_glow detaches from that node even if the slot's occupant later
# changes.
...
# state — the host turns it on for the enemy a selected/dragged friendly unit will strike.
...
func set_card(card: CardUI) -> void:
	...
	# handling), which marks the event handled and stops it from ever bubbling to this slot's own
	# _gui_input — MOUSE_FILTER_PASS doesn't help, since Godot only forwards an event to the
	# parent if the child left it unhandled. So we listen to the card's `pressed` signal directly
	# instead of relying on GUI event propagation.
...
func clear_card() -> CardUI:
	var card := _card_ui
	# The ground's light stops when the unit steps out of it. Only this path needs to say so — a
	# card moving to another SLOT is re-dressed by that slot's own layout, but one leaving the
	# board entirely (picked up, killed) has nobody left to speak for it and would carry the
	# ground's colour away.
	if card != null and is_instance_valid(card) and card.has_method(&"set_ground_tint"):
...
func _can_drop_data(_at: Vector2, data: Variant) -> bool:
	...
	# all — not even an invalid one: reporting nothing leaves the ghost on its default view,
	# which is the honest reading of "this slot is not answering you."
