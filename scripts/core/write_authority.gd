...
static func stat_write(entity: GameEntity, stat: StringName, value: float,
	...
	if stat == &"health" and previous > 0.0 and committed <= 0.0:
		# Sources (Core §8): died carries the request's source — the holder of the
		# effect that minted the request. The dead entity is the event's native
		# target; the request at hand rides as RequestEventData.
