...
function gameAttrDefaults() {
  return { 'mana.initial': 1, 'mana.max': 10, 'mana.per_turn': 0, 'hand.size.initial': 3,
    ...
    'bounty.gold_per_cost': 1, 'bounty.exp_per_cost': 1, 'bounty.minimum': 1,
...
function gameAttributesSection(cfg) {
  ...
  root = el('div', { class: 'panel tuning-section' },
    ...
    el('div', { style: 'display:flex;gap:26px;flex-wrap:wrap;margin-top:12px' },
      ...
      col('Kill bounties',
        row('bounty.gold_per_cost', 'Gold per mana cost', 'gold a slain enemy pays per point of its mana cost — one coin flies to the purse per gold', 0, 10, 0.5),
        row('bounty.exp_per_cost', 'Experience per mana cost', 'profile experience a slain enemy pays per point of its mana cost', 0, 10, 0.5),
        row('bounty.minimum', 'Minimum bounty', 'floor for a derived bounty, so a 0-cost body is still worth killing (a card may author its own flat bounty, which ignores all of this)', 0, 10, 1)),
      col('Forge & shop',
