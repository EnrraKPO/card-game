...
const PERSONALITY_TRAITS = [
  ...
  { id: 'formation', core: true, def: 0.1, max: 1, label: 'Best seats for the valuable',
    sub: 'worth order matches cover order',
    hint: 'Higher value units deserve the best seats — the better covered slots. It compares its units pair by pair: worth against the seat\'s exposure (the same covered-ness number the combat log prints per slot), and scores the fraction seated the right way round. A misordered pair costs proportionally, never absolutely — a tank holding the front for everyone loses only its own pairs, so suboptimal setups still count. This is the only voice left when a big body soaks the whole incoming pour and every slot behind it reads zero damage — the hedge against what leaks through anyway — and the voice that makes reshuffling worth doing. Loudest on a sparse board, where a prize parked in front of a fodder is a blatant offence.' },
