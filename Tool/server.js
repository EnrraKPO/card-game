...
const PERSONALITY_TRAITS = [
  ...
  { id: 'formation', core: true, def: 0.1, max: 1, label: 'Protection chain',
    sub: 'each prize screened by the next body down',
    hint: 'Sorts its units by worth and asks of each link: is this unit screened by the next cheaper one — a body in a strictly nearer column? Full marks per protected link, half for sharing a column, nothing for standing in front of your own protector. The cheapest body belongs at the true front and the prize behind the whole procession; protection is BUILT out of bodies, never found in a deep coordinate. It has no opinion about how deep the army sits, only about who stands between whom and the enemy. This is the only voice left when a big body soaks the whole incoming pour and every slot behind it reads zero — the hedge against the damage that leaks through anyway. Loudest on a sparse board, where a prize parked in front of a fodder is a blatant offence.' },
