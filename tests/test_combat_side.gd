...
func _draw_form() -> void:
	...
	var out := Arbitrator.submit(StatMutation.make(s, StatMutation.DRAW, 2))
	...
	var over := Arbitrator.submit(StatMutation.make(s, StatMutation.DRAW, 5))
	check_eq(over.delta, 1, "drawing past the pile stops at it (delta reports what moved)")
	var empty := Arbitrator.submit(StatMutation.make(s, StatMutation.DRAW, 3))
	check_eq(empty.delta, 0, "an empty pile draws nothing")
	var negative := Arbitrator.submit(StatMutation.make(s, StatMutation.DRAW, -2))
...
func _discard_form() -> void:
	...
	var out := Arbitrator.submit(StatMutation.make(s, StatMutation.DISCARD, 2))
	check_eq(out.delta, 2, "discard 2 removes 2")
	check_eq(s.hand.size(), 1, "the hand shrank to 1")
	var over := Arbitrator.submit(StatMutation.make(s, StatMutation.DISCARD, 5))
	check_eq(over.delta, 1, "discarding past the hand stops at it")
	check_eq(s.hand.size(), 0, "hand empty")
	check_eq(Arbitrator.submit(StatMutation.make(s, StatMutation.DISCARD, 1)).delta, 0,
...
func _max_mana_form() -> void:
	...
	var floor_out := Arbitrator.submit(StatMutation.make(s, StatMutation.MAX_MANA, -5))
